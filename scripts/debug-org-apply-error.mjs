 ...
 * Usage: node scripts/debug-org-apply-error.mjs <organizationId>
 */
...
async function debugOrganization(organizationId) {
  ...
  try {
    ...
    if (!orgData?.liffId) {
      ...
      console.log(`   Has leading/trailing spaces: ${orgData.liffId !== orgData.liffId.trim() ? '⚠️ YES' : 'No'}`);
    ...
    if (eventsSnapshot.empty) {
      ...
      eventsSnapshot.docs.forEach(doc => {
        ...
        if (eventData.slots && eventData.slots.length > 0) {
          eventData.slots.forEach((slot, index) => {
